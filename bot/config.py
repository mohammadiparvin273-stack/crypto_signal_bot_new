...
# نکته‌ی مهم (اصلاح‌شده): قبلاً ADX و هم‌جهتی EMA/RSI به‌صورت فیلتر سخت (رد کامل سیگنال) اعمال
# می‌شدن، جدا از این امتیازدهی. این باعث می‌شد چند فیلتر AND پشت‌سرهم (روند بزرگ + EMA/RSI +
# ADX + امتیاز + AI) هم‌زمان رد بشن و برای مدت طولانی هیچ سیگنالی رد نشه، حتی وقتی حرکت بزرگی
# در بازار در جریان بود. الان همون دو مورد به شکل امتیاز نرم (momentum) اضافه شدن تا کیفیت رو
# پایین نیارن ولی دیگه به‌تنهایی کل سیگنال رو کور نکنن.
SCORE_WEIGHTS = {
    "trend": 22,
    "volume": 15,
    "news": 5,
    "liquidity": 20,
    "order_block": 8,  # توی بک‌تست همبستگی نزدیک صفر یا منفی داشت (-0.099, -0.166)
    "rsi": 5,
    "open_interest": 10,
    "funding": 5,
    "momentum": 10,  # جایگزین نرم فیلترهای سخت قبلی ADX و EMA/RSI (به‌جای رد کامل سیگنال)
}
TOTAL_SCORE = sum(SCORE_WEIGHTS.values())  # = 100

SCORE_GOLDEN = float(os.environ.get("SCORE_GOLDEN", "90"))   # سیگنال طلایی
SCORE_GOOD = float(os.environ.get("SCORE_GOOD", "80"))       # سیگنال خوب
SCORE_MIN_SEND = float(os.environ.get("SCORE_MIN_SEND", "65"))  # کمتر از این اصلا ارسال نشود
# (SCORE_MIN_SEND از ۷۰ به ۶۵ کاهش یافت چون momentum و order_block دیگه به‌صورت فیلتر سخت جدا
# اعمال نمی‌شن، خودشون داخل امتیاز حساب میشن؛ اگه می‌خوای سخت‌گیرانه‌تر باشه، با env override کن)
...
# هدف/آستانه‌ی «قدرت روند خوب» برای امتیازدهی نرم ADX (نه فیلتر سخت). هرچی ADX به این عدد
# نزدیک‌تر یا بیشتر باشه، امتیاز momentum بیشتر می‌گیره.
ADX_MIN_TREND_STRENGTH = float(os.environ.get("ADX_MIN_TREND_STRENGTH", "20"))
# کف مطلق ADX: فقط زیر این مقدار (یعنی واقعاً بازار کاملاً بی‌روند/رنج خنثی) سیگنال رد میشه.
# این جایگزین فیلتر سخت قبلی (رد کامل زیر ۲۰) شده تا حرکت‌های تازه‌شروع‌شده (که ADX هنوز کامل
# بالا نیومده) از دست نرن.
ADX_HARD_FLOOR = float(os.environ.get("ADX_HARD_FLOOR", "10"))

# وزن هر تایم‌فریمِ روند بزرگ در تشخیص جهت غالب (نزدیک‌تر به لحظه‌ی حال = وزن بیشتر، چون به
# حرکت‌های تازه سریع‌تر واکنش نشون می‌ده). قبلاً هر سه تایم‌فریم وزن مساوی داشتن که باعث می‌شد
# وقتی روند هفتگی/روزانه هنوز عقب بودن ولی ۴ساعته به‌وضوح برگشته بود، جهت غالب اشتباه (یا خنثی)
# تشخیص داده بشه و کل سیگنال رد بشه - دقیقاً همون چیزی که باعث جا موندن از حرکت‌های بزرگ میشه.
TREND_TF_WEIGHTS = {"1w": 1, "1d": 2, "4h": 3}

# اگه بعد از این‌همه ساعت هیچ سیگنالی ارسال نشده باشه، یه پیام تشخیصی (خلاصه‌ی دلایل رد شدن)
# به تلگرام فرستاده میشه تا لازم نباشه دستی JSON اندپوینت اسکن رو چک کنی.
SILENCE_ALERT_HOURS = float(os.environ.get("SILENCE_ALERT_HOURS", "6"))
