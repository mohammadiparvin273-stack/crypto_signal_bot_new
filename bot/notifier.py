...
def format_signal_message(signal: dict) -> str:
    ...
    sl_source = signal["trade_plan"].get("stop_loss_source", "atr")
    sl_label = "ساختاری - پشت Order Block" if sl_source == "structural" else "بر مبنای ATR"
    lines = [
        ...
        f"حد ضرر (SL): {signal['trade_plan']['stop_loss']} ({sl_label})",
...
# ---------- توابع تعاملی تلگرام (دکمه‌ها، برای ورود دستی/تاییدشده به معامله) ----------

def send_message_with_keyboard(chat_id: str, text: str, keyboard_rows: list) -> int | None:
    """
    ارسال پیام با دکمه‌های inline. keyboard_rows: لیستی از ردیف‌ها، هر ردیف لیستی از
    {"text": "...", "callback_data": "..."}.
    """
    if not TELEGRAM_BOT_TOKEN or not chat_id:
        logger.warning("تلگرام یا chat_id تنظیم نشده - پیام تعاملی ارسال نشد.")
        return None
    url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
    payload = {
    ...
    try:
        ...
        if data.get("ok"):
            return data["result"]["message_id"]
        logger.error("ارسال پیام تعاملی شکست خورد: %s", data)
        return None
    except Exception as e:
        logger.error("خطا در ارسال پیام تعاملی: %s", e)
        return None


def edit_message_with_keyboard(chat_id: str, message_id: int, text: str, keyboard_rows: list | None = None):
    ...
    try:
        ...
        logger.warning("ویرایش پیام شکست خورد: %s", e)
...
def answer_callback_query(callback_query_id: str, text: str = ""):
    ...
    try:
        ...
        logger.warning("پاسخ به callback شکست خورد: %s", e)


def send_trade_entry_prompt(signal: dict, signal_id: str, chat_id: str):
    """پیام تعاملی «ورود به معامله» با دکمه‌های انتخاب مبلغ - توی چت شخصی (نه کانال) فرستاده میشه"""
    from bot.config import AMOUNT_PRESETS_USDT
    direction_fa = "لانگ 📈" if signal["direction"] == "long" else "شورت 📉"
    text = (
        f"💰 ورود به معامله — {signal['symbol']} ({direction_fa})\n\n"
        f"ورود: {signal['trade_plan']['entry']} | SL: {signal['trade_plan']['stop_loss']} | "
        f"TP1: {signal['trade_plan']['take_profit_1']}\n\n"
        f"چقدر (دلار مارجین) می‌خوای وارد بشی؟"
    )
    row1 = [{"text": f"${amt}", "callback_data": f"amt:{signal_id}:{amt}"} for amt in AMOUNT_PRESETS_USDT]
    row2 = [{"text": "✏️ مبلغ دلخواه", "callback_data": f"amtcustom:{signal_id}"}]
    row3 = [{"text": "❌ رد کردن این معامله", "callback_data": f"cancel:{signal_id}"}]
    return send_message_with_keyboard(chat_id, text, [row1, row2, row3])
