        ...
        for symbol in symbols:
            for tf in timeframes:
                try:
                    ...
                    # تایم‌فریم‌های متراکم‌تر (کندل بیشتر) با step بزرگ‌تر بررسی میشن تا زمان اجرا معقول بمونه
                    auto_step = 4 if tf in ("5m", "15m") else (2 if tf in ("30m", "1h") else 1)
                    recs = simulate_strategy(df, step=auto_step)
