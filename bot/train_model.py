...
def train_and_save(records: list, sample_weight: list | None = None):
    X = np.array([build_feature_vector(r["features"]) for r in records])
    ...
    # تقسیم زمانی (Walk-Forward) به‌جای تصادفی: چون داده‌ی مالی/زمانیه، تقسیم تصادفی می‌تونه باعث
    # «نشتی داده» بشه (مدل غیرمستقیم از آینده به گذشته نگاه کنه). این‌جا همیشه آخرین ۲۰٪ داده
    # (از نظر زمانی/ترتیب تولید) به‌عنوان تست جدا میشه - دقیقاً شبیه‌سازی واقعی: آموزش با گذشته،
    # تست با آینده.
    ...
    if len(set(y_test)) < 2:
        logger.warning(
            "بخش تست (۲۰٪ آخر به ترتیب زمانی) فقط یک کلاس داره - احتمالاً چون داده‌ها یکجا از "
            "چند نماد/تایم‌فریم جدا پشت‌سرهم اومدن، نه واقعاً یکپارچه زمانی. دقت AUC این دور "
            "قابل‌اتکا نیست؛ نتیجه رو با احتیاط ببین."
        )
