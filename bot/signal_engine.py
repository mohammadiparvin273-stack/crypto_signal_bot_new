...
from bot.config import TREND_TIMEFRAMES, ENTRY_TIMEFRAMES, CANDLES_LIMIT, PRIMARY_EXCHANGE, MIN_AI_PROBABILITY_PCT
...
def build_entry_reason(direction: str, pa_report: dict, smc_report: dict, ict_report: dict,
                        vwap_aligned: bool | None = None) -> str:
    reasons = []
...
def analyze_symbol(symbol: str, direction_tally: dict | None = None, rejection_stats: dict | None = None) -> list:
    ...
    for tf in ENTRY_TIMEFRAMES:
        ...
        reason = build_entry_reason(direction, pa_report, smc_report, ict_report, vwap_aligned)
