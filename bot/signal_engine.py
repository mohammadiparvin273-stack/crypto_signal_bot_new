...
def determine_big_trend(mtf_data: dict) -> dict:
    """
    روند بزرگ از تایم‌فریم‌های TREND_TIMEFRAMES با کمک EMA50/EMA200 و Supertrend تشخیص داده میشه.
    ...
    for tf in TREND_TIMEFRAMES:
        ...
        ema50, ema200 = ind["ema_50"].iloc[-1], ind["ema_200"].iloc[-1]
        st_dir = ind["supertrend_dir"].iloc[-1]
        votes_up = sum([ema50 > ema200, st_dir == 1])
        trends[tf] = "up" if votes_up >= 1 and ema50 > ema200 else ("down" if ema50 < ema200 else None)
    return trends
