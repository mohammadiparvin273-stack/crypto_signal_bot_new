...
def analyze_symbol(symbol: str, direction_tally: dict | None = None) -> list:
    """
    ...
    for tf in ENTRY_TIMEFRAMES:
        ...
        # فیلتر Volume Profile: برای لانگ، ورود نزدیک/زیر POC (منطقه‌ی معقول، نه دنبال‌کردن حرکت افراطی
        # بالای POC) بهتره؛ برای شورت برعکس. این از Volume Profile به‌عنوان یه تاییدیه‌ی مستقل استفاده
        # می‌کنه، نه فقط جزئی گمشده توی امتیاز کلی.
        entry_price_check = float(df["close"].iloc[-1])
        vp_profile = volume_report.get("profile")
        if vp_profile and vp_profile.get("poc_price"):
            poc = vp_profile["poc_price"]
            volume_profile_ok = (entry_price_check <= poc * 1.01) if direction == "long" else (entry_price_check >= poc * 0.99)
...
def scan_all_symbols(symbols: list) -> list:
    ...
    for symbol in symbols:
        ...
        if elapsed > MAX_SCAN_SECONDS:
            ...
            break
        try:
            sigs = analyze_symbol(symbol, direction_tally=direction_tally)
