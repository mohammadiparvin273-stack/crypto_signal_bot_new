...
def simulate_strategy(df: pd.DataFrame, window: int = 250, rr_target_index: int = 1, holding_max: int = 50, step: int = 1):
    """
    ...
    `step`: هر چند کندل یک‌بار بررسی بشه (۱=همه، ۳=هر سومی). چون تحلیل‌های پرایس‌اکشن/SMC روی
    کندل‌های پشت‌سرهم شباهت زیادی دارن، افزایش step سرعت رو چند برابر می‌کنه بدون افت محسوس کیفیت.
    ...
    for i in range(window, len(df) - holding_max, step):
