...
from bot.price_action import full_price_action_report
...
def simulate_strategy(df: pd.DataFrame, window: int = 250, rr_target_index: int = 1, holding_max: int = 50):
    ...
    for i in range(window, len(df) - holding_max):
        ...
        direction = "long" if close.iloc[i] > ema200.iloc[i] else "short"
        ...
        volume_report = {"volume_spike": {"is_current_spike": False}, "delta": {"last_delta": 0}}
        ...
        score = compute_final_score(direction, mtf_trends, volume_report, sentiment_report,
                                     smc_report, pa_report, float(rsi_series.iloc[i]), derivatives_report)
...
def compute_backtest_metrics(records: list) -> dict:
    if not records:
        return {"trades": 0}
    wins = [r for r in records if r["label"] == 1]
    losses = [r for r in records if r["label"] == 0]
    win_rate = len(wins) / len(records) * 100
    gross_profit = sum(r["rr"] for r in wins)
    gross_loss = len(losses)  # هر ضرر معادل ۱ واحد ریسک
    profit_factor = (gross_profit / gross_loss) if gross_loss else float("inf")

    returns = [r["rr"] if r["label"] == 1 else -1 for r in records]
