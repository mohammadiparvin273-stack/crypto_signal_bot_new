...
def detect_liquidity_grab(df: pd.DataFrame, left=3, right=3, wick_ratio=0.6):
    """
    ...
    for i in range(len(df)):
        ...
        for _, sh in recent_highs.iterrows():
            if sh.name >= i:
                continue
            if row["high"] > sh["high"] and row["close"] < sh["high"] and (upper_wick / candle_range) >= wick_ratio:
                grabs.append({"type": "sell_side_liquidity_grab", "index": i, "swept_level": float(sh["high"]),
                              "note": "Stop Hunt بالای سقف قبلی - احتمال برگشت به سمت پایین"})

        for _, sl in recent_lows.iterrows():
            if sl.name >= i:
                continue
            if row["low"] < sl["low"] and row["close"] > sl["low"] and (lower_wick / candle_range) >= wick_ratio:
                grabs.append({"type": "buy_side_liquidity_grab", "index": i, "swept_level": float(sl["low"]),
                              "note": "Stop Hunt پایین کف قبلی - احتمال برگشت به سمت بالا"})
