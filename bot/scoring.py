...
"""
from bot.config import SCORE_WEIGHTS, SCORE_GOLDEN, SCORE_GOOD, SCORE_MIN_SEND, ADX_MIN_TREND_STRENGTH, ADX_HARD_FLOOR
...
def score_volume(volume_report: dict, direction: str = "long", entry_price: float | None = None,
                  order_book_imbalance: float | None = None) -> float:
    """
    وزن حجم بین چهار بخش تقسیم میشه: Volume Spike (۳۰٪)، Delta تخمینی (۲۵٪)،
    موقعیت نسبت به Volume Profile/POC (۲۵٪)، و عدم‌تعادل Order Book (۲۰٪ - نسخه‌ی رایگان
    Order Flow؛ اگه فشار خرید/فروش دفتر سفارش هم‌جهت سیگنال باشه، بونوس می‌گیره).
    """
    ...
    if delta > 0:
        score += weight * 0.25 * min(1.0, abs(delta) / (abs(delta) + 1))
    ...
    if vp_profile and vp_profile.get("poc_price") and entry_price:
        ...
        score += weight * 0.25 if aligned else weight * 0.08
    else:
        score += weight * 0.12  # داده‌ی کافی نیست - امتیاز خنثی/متوسط

    if order_book_imbalance is not None:
        # imbalance مثبت یعنی فشار خرید بیشتره؛ منفی یعنی فشار فروش بیشتره
        aligned_ob = (order_book_imbalance > 0.05) if direction == "long" else (order_book_imbalance < -0.05)
        if aligned_ob:
            score += weight * 0.2 * min(1.0, abs(order_book_imbalance) / 0.3)
        else:
            score += weight * 0.05
    else:
        score += weight * 0.1  # داده‌ی کافی نیست - امتیاز خنثی/متوسط
...
def score_liquidity(smc_report: dict, direction: str, vwap_aligned: bool | None = None) -> float:
    ...
    # بونوس VWAP: معامله‌گران روزانه/موسسات معمولاً VWAP رو به‌عنوان یه سطح نقدینگی/میانگین
    # منصفانه در نظر می‌گیرن. اگه قیمت هم‌جهت با VWAP باشه (بالای VWAP برای لانگ، زیرش برای شورت)،
    # یه بونوس کوچیک اضافه میشه.
    if vwap_aligned is True and base < 1.0:
        base = min(1.0, base + 0.15)

    return round(weight * base, 2)
...
def score_order_block(price_action_report: dict, direction: str) -> float:
    ...
    # بونوس Breakout+Retest: حتی اگه Order Block مستقیم نداشتیم، یه الگوی
    # شکست+ریتست معتبر هم‌جهت، تقریباً به همون اندازه معتبره.
    br = price_action_report.get("breakout_retest")
    wanted_br = "bullish_breakout_retest" if direction == "long" else "bearish_breakout_retest"
    if br and br.get("type") == wanted_br:
...
def score_momentum(adx_value: float, ema_aligned: bool, rsi_aligned: bool) -> float:
    """
    جایگزین نرمِ فیلترهای سختِ قبلی ADX و هم‌جهتیِ EMA20/EMA50+RSI.
    قبلاً این دو تا فیلتر AND جدا بودن (نه بخشی از امتیاز) و اگه رد می‌شدن، سیگنال کامل
    از بین می‌رفت - حالا فقط به‌عنوان یه امتیاز کمکی حساب میشن، پس یه سیگنال قوی از نظر
    ساختار/نقدینگی/حجم می‌تونه حتی با ADX یا هم‌جهتی ضعیف‌تر هم رد بشه (نه لزوماً حذف کامل).
    ۶۰٪ وزن -> قدرت روند (ADX) نسبت به آستانه‌ی هدف. ۴۰٪ وزن -> هم‌جهتی EMA/RSI.
    """
    weight = SCORE_WEIGHTS["momentum"]
    adx_range = max(ADX_MIN_TREND_STRENGTH - ADX_HARD_FLOOR, 1e-6)
    adx_ratio = min(max((adx_value - ADX_HARD_FLOOR) / adx_range, 0.0), 1.0)
    adx_score = weight * 0.6 * adx_ratio

    aligned_count = int(ema_aligned) + int(rsi_aligned)
    confluence_ratio = {0: 0.1, 1: 0.5, 2: 1.0}[aligned_count]
    confluence_score = weight * 0.4 * confluence_ratio

    return round(adx_score + confluence_score, 2)
...
def compute_final_score(direction: str, mtf_trends: dict, volume_report: dict, sentiment_report: dict,
                          ...
                          derivatives_report: dict, entry_price: float | None = None,
                          vwap_aligned: bool | None = None, order_book_imbalance: float | None = None,
                          adx_value: float = 0.0, ema_aligned: bool = False, rsi_aligned: bool = False) -> dict:
    breakdown = {
        "trend": score_trend(mtf_trends, direction),
        "volume": score_volume(volume_report, direction, entry_price, order_book_imbalance),
        "news": score_news(sentiment_report, direction),
        "liquidity": score_liquidity(smc_report, direction, vwap_aligned),
        ...
        "momentum": score_momentum(adx_value, ema_aligned, rsi_aligned),
    }
