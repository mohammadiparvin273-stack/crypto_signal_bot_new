...
def acquire_lock(key: str = "scan_lock", ttl_seconds: int = 600) -> bool:
    """
    قفل اتمیک با Upstash: فقط اگه کلید از قبل وجود نداشته باشه ست میشه (NX)، با انقضای
    خودکار بعد از ttl_seconds (EX) - برای این‌که اگه یه اجرا کرش کرد، قفل تا ابد گیر نکنه.
    ...
    result = _request("set", key, "locked", "EX", str(ttl_seconds), "NX")
    return result is not None  # اگه set موفق بود (نتیجه OK بود)، یعنی قفل قبلاً نبوده
