...
# ---------- قفل اتمیک (جلوگیری از اجرای هم‌زمان چند اسکن که باعث سیگنال تکراری میشه) ----------

def delete_value(key: str) -> bool:
    result = _request("del", key)
    return result is not None


def acquire_lock(key: str = "scan_lock", ttl_seconds: int = 600) -> bool:
    """
    قفل ساده مبتنی بر timestamp (به‌جای دستور NX/EX مستقیم که سینتکسش توی Upstash REST
    نامطمئن بود و باعث میشد قفل همیشه "گرفته‌شده" تشخیص داده بشه و اسکن هیچ‌وقت اجرا نشه).
    این نسخه از همون get_value/set_value همیشه-کارکن استفاده می‌کنه.
    خروجی: True یعنی قفل گرفته شد (اجازه‌ی ادامه داری)، False یعنی یه اجرای دیگه در حال انجامه.
    """
    if not _ENABLED:
        return True  # اگه storage غیرفعاله، قفل رو نادیده می‌گیریم (بهتر از این‌که کل ربات متوقف بشه)
    import time
    now = time.time()
    existing = get_value(key, default=0) or 0
    if existing and (now - existing) < ttl_seconds:
        return False  # یه اجرای دیگه اخیراً قفل رو گرفته و هنوز منقضی نشده
    set_value(key, now)
    return True


def release_lock(key: str = "scan_lock"):
    if not _ENABLED:
        return
    delete_value(key)
