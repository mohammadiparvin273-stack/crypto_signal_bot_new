...
from bot.config import MAX_DAILY_LOSS_PCT, MAX_WEEKLY_LOSS_PCT, MAX_DRAWDOWN_PCT, MAX_PORTFOLIO_OPEN_RISK_PCT, MAX_SAME_DIRECTION_OPEN, SILENCE_ALERT_HOURS  # noqa: E402
from bot.signal_engine import scan_all_symbols  # noqa: E402
from bot.notifier import broadcast_signal, broadcast_news_text, format_news_digest, send_telegram_message  # noqa: E402
...
def _maybe_send_silence_alert(sent: list, rejection_stats: dict, direction_tally: dict, now: float):
    """
    اگه مدت زیادی (SILENCE_ALERT_HOURS) هیچ سیگنالی ارسال نشده، خلاصه‌ی دلایل رد شدن رو
    یه‌بار به تلگرام می‌فرسته - تا لازم نباشه دستی JSON اندپوینت /api/scan رو چک کنی که
    "الان چرا سیگنال نمیاد". حداکثر هر SILENCE_ALERT_HOURS یه‌بار ارسال میشه (نه هر اسکن).
    """
    if sent:
        storage.set_value("last_any_signal_sent_ts", now)
        return
    last_sent_ts = storage.get_value("last_any_signal_sent_ts", default=now)
    hours_silent = (now - last_sent_ts) / 3600
    if hours_silent < SILENCE_ALERT_HOURS:
        return
    last_alert_ts = storage.get_value("last_silence_alert_ts", default=0.0) or 0.0
    if (now - last_alert_ts) / 3600 < SILENCE_ALERT_HOURS:
        return  # همین اواخر یه بار هشدار داده شده، دوباره اسپم نکن
    top_reasons = sorted(rejection_stats.items(), key=lambda x: x[1], reverse=True)[:6]
    lines = [
        f"⏳ حدود {round(hours_silent, 1)} ساعته هیچ سیگنالی ارسال نشده.",
        "",
        f"تشخیص روند این اجرا: {direction_tally}",
        "بیشترین دلایل رد شدن:",
    ]
    for reason, count in top_reasons:
        lines.append(f"• {reason}: {count} مورد")
    lines.append("")
    lines.append("این پیام صرفاً تشخیصیه؛ اگه بازار واقعاً رنج/بی‌روند باشه طبیعیه، وگرنه آستانه‌ها "
                 "(SCORE_MIN_SEND / ADX_HARD_FLOOR) رو در تنظیمات محیطی بررسی کن.")
    send_telegram_message("\n".join(lines))
    storage.set_value("last_silence_alert_ts", now)
...
def _run_scan_locked(symbols):
    try:
        ...
        try:
            _maybe_send_silence_alert(sent, rejection_stats, direction_tally, now)
        except Exception as e:
            app.logger.error("ارسال هشدار سکوت شکست خورد: %s", e)
