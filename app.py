...
<style>
  ...
  .card { background: #1a1d29; border-radius: 10px; padding: 14px; text-align: center; }
  .card .label { font-size: 12px; color: #9aa0ac; margin-bottom: 6px; }
  .card .value { font-size: 22px; font-weight: bold; }
  .positive { color: #4caf50; }
  .negative { color: #f44336; }
  table { width: 100%; border-collapse: collapse; font-size: 13px; margin-top: 10px; }
  th, td { padding: 8px 6px; text-align: right; border-bottom: 1px solid #2a2e3d; }
...
async function loadDashboard() {
  ...
  try {
    ...
    html += `<div class="card"><div class="label">Win Rate واقعی</div><div class="value ${pnlClass(data.real_win_rate_pct - 50)}">${fmt(data.real_win_rate_pct, '%')}</div></div>`;
    ...
    html += `<div class="card"><div class="label">بهترین ارز</div><div class="value" style="font-size:16px">${fmt(data.best_symbol_by_avg_score)}</div></div>`;
    html += `<div class="card"><div class="label">بدترین ارز</div><div class="value" style="font-size:16px">${fmt(data.worst_symbol_by_avg_score)}</div></div>`;
    html += '</div>';

    html += '<table><thead><tr><th>ارز</th><th>جهت</th><th>تایم‌فریم</th><th>امتیاز</th><th>وضعیت</th><th>سود/ضرر</th></tr></thead><tbody>';
    const rows = (data.recent_signals || []).slice().reverse();
    for (const t of rows) {
      const dirBadge = t.direction === 'long' ? '<span class="badge long">لانگ</span>' : '<span class="badge short">شورت</span>';
      let statusCell;
      if (t.status === 'open') {
        statusCell = '<span class="badge open">باز</span>';
...
def weekly_report():
    ...
    try:
        ...
        wins = [t for t in closed if str(t.get("outcome", "")).startswith("take_profit")]
        ...
        for t in recent:
            ...
            if t.get("outcome", "").startswith("take_profit"):
