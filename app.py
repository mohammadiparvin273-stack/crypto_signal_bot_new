...
from bot.config import MAX_DAILY_LOSS_PCT, MAX_WEEKLY_LOSS_PCT, MAX_DRAWDOWN_PCT, MAX_PORTFOLIO_OPEN_RISK_PCT  # noqa: E402
    ...
    try:
        ...
        for sig in deduped_signals:
            ...
            if current_open_risk_pct + RISK_PER_TRADE_PCT > MAX_PORTFOLIO_OPEN_RISK_PCT:
                skipped_portfolio_risk.append(symbol)
                continue
...
def dashboard():
    try:
        ...
        def _direction_stats(direction):
            ...
            d_wins = [t for t in d_closed if t.get("outcome", "").startswith("take_profit")]
            return {
                "count": len(d_closed),
                "wins": len(d_wins),
